...
public:
	...
	Vector CalculateLightModel(Vector Point, Vector n, Vector v, LightConstants Material, vector<Shapes>& ShapeList, int CurrentShape)
	{
	...
	float MicroFacet(Vector l, Vector v, Vector n, float m)
	{
		...
		float Beckmann = ( 1 / ( 4.0f * ( m * m) * pow( (cos(J)), 4))) * exp( (-1.0f * (tan(J) * tan(J))) / (m * m));
		return Beckmann;
	};	
	...
	{	
		...
		Vector h = (l + v) / Norm(l + v);
		Vector n0 = (l * DotProduct(n,l)) + (v * DotProduct(n,v)) - (l * ( DotProduct(v,l) * DotProduct(v,n) )) - ( v * ( DotProduct(v,l) * DotProduct(l,n)) );
		if(Norm(n0) != 0)
		{
			m = n0 / Norm(n0);
		}
